...
<div class="card">
    <div class="card-body">
        <div class="row">
        <div class="col-md-3">
                <label>Type Calculation</label>
                <select class="form-control" wire:model.defer="searchDosageFormTypeId">
                    <option value="">All</option>
                    @foreach($dosageFormTypeName as $dosageform)
                    <option value="{{$dosageform->id}}">{{$dosageform->dosage_form_type_name}}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-3">
                <label>Dosage Form Name</label>
                <input type="text" placeholder="Dosage Form" class="form-control" wire:model.defer="searchDosageFormName">
            </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="col-md-6 col-sm-6 mb-3">
                    <label>Dosage Form Type<span class="req">*</span></label>
                    <select class="form-control @error('dosage_form_type_id') is-invalid @enderror"  wire:model.defer="dosage_form_type_id">
                        <option value="">Select Type</option>
                        @foreach($dosageFormTypeName as $dosageform)
                            <option value="{{$dosageform->id}}">{{$dosageform->dosage_form_type_name}}</option>
                        @endforeach
                    </select>
                    @error('dosage_form_type_id')
                        <em id="name-error" class="error invalid-feedback">{{ $message }}</em>
                        @enderror
                    </div>
                    <div class="col-md-6 col-sm-6 mb-3">
                        <label>Dosage Form Name <span class="req">*</span></label>
                        <div>
                            <input type="text" maxlength="100" class="form-control @error('dosage_form_name') is-invalid @enderror" wire:model.defer="dosage_form_name" placeholder="Dosage Form Name" />
                            @error('dosage_form_name')
                            <em id="name-error" class="error invalid-feedback">{{ $message }}</em>
                            @enderror
                        </div>
                    </div>
